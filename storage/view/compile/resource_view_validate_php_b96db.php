...
<div class="container-fluid">
	<div
		style="background: url('<?php echo __ROOT__?>/resource/images/logo.png') no-repeat;background-size: contain;height:80px;margin-top: 60px;"></div>
	<div class="alert alert-info clearfix jumbotron" style="padding:30px;margin-top: 50px;">
		...
		<div class="col-xs-9">
			<p>
				<?php if(is_array($errors) || is_object($errors)){foreach ($errors as $v){?><?php echo $v?><br/><?php }}?>
			</p>
			<p>
				<a href="<?php echo __HISTORY__?>" class="alert-link">如果你的浏览器没有跳转, 请点击此链接</a>
			</p>
		</div>
	</div>
</div>
<script>
	setTimeout(function () {
		location.href = '<?php echo __HISTORY__?>';
	}, 3000);
</script>
</body>
</html>